...
function TestimonialSection() {
  ...
  return (
    <section className="relative py-16 sm:py-20 lg:py-24 bg-white dark:bg-[#0f1f32]">
      {/* subtle radial glow on navy */}
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10">
        <div className="hidden dark:block w-full h-full" style={{ background: "radial-gradient(650px 420px at 55% 40%, rgba(72, 123, 164, 0.20) 0%, rgba(72,123,164,0.10) 30%, transparent 65%)" }} />
        <div className="block dark:hidden w-full h-full" style={{ background: "radial-gradient(600px 380px at 55% 40%, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0.03) 30%, transparent 60%)" }} />
      </div>

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-12 text-center sm:mb-16">
          <div className="mb-4 inline-flex items-center space-x-2 rounded-full px-4 py-2 text-sm font-medium text-emerald-600 bg-emerald-50 border border-emerald-100 dark:text-emerald-300 dark:bg-slate-900/60 dark:border-slate-800">
            <Heart className="h-4 w-4" />
            <span>{"Patient Reviews"}</span>
          </div>
          <h2 className="mb-4 text-3xl font-bold text-gray-900 sm:text-4xl lg:text-5xl dark:text-white">{"Our Patients Speak"}</h2>
          <p className="mx-auto max-w-3xl text-lg text-gray-600 sm:text-xl dark:text-slate-300">
        ...
        <div className="mb-12 grid grid-cols-3 gap-4 sm:mb-16 sm:gap-8">
          {features.map((feature, index) => (
            ...
            >
              <div
                className={`mb-3 inline-flex h-12 w-12 items-center justify-center rounded-full shadow-lg sm:mb-4 sm:h-16 sm:w-16 ${feature.color} bg-emerald-50 dark:bg-slate-900/70`}
              >
                <feature.icon className="h-6 w-6 sm:h-8 sm:w-8" />
              </div>
              <div className="mb-1 text-xl font-bold text-gray-900 sm:text-2xl lg:text-3xl dark:text-white">{feature.title}</div>
              <div className="text-xs text-gray-600 sm:text-sm dark:text-slate-300">{feature.subtitle}</div>
            </div>
          ))}
        </div>
        ...
        >
          <div className="relative">
            ...
            <div className="px-4 py-2">
              <Card className={`group h-full border bg-white shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl dark:border-0 dark:bg-slate-800/60 dark:backdrop-blur-sm ${isTransitioning ? "opacity-0 scale-95" : "opacity-100 scale-100"}`}>
                <CardContent className="relative p-6 sm:p-8">
                  ...
                  {/* Review Text */}
                  <p className="mb-6 text-center italic leading-relaxed text-gray-700 text-lg sm:mb-8 dark:text-slate-200">
                    {'"'}
                    {testimonial.text}
                    {'"'}
                  </p>
                  ...
                  <div className="text-center">
                    ...
                    <h4 className="text-lg font-bold text-gray-900 sm:text-xl dark:text-white">{testimonial.name}</h4>
                    <p className="mb-2 text-sm text-gray-600 sm:text-base dark:text-slate-300">{testimonial.role}</p>
                    <div className="mb-2 flex items-center justify-center space-x-1 text-sm text-gray-600 dark:text-slate-300">
                      <MapPin className="h-4 w-4" aria-hidden="true" />
                      <span>{testimonial.location}</span>
                    </div>
                    <div className="flex items-center justify-center space-x-2 text-sm">
                      <span className="rounded-full px-3 py-1 text-emerald-700 bg-emerald-50 dark:bg-slate-800/50 dark:text-emerald-300">
                        {testimonial.specialty}
                      </span>
                      <span className="text-gray-400 dark:text-slate-500">{"•"}</span>
                      <span className="text-gray-600 dark:text-slate-300">{testimonial.date}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
